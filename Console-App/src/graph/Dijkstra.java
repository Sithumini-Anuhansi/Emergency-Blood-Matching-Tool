...
public class Dijkstra 
{
    ...
    // Find shortest distance from source to all locations
    public Map<Location, Double> shortestDistances(Location source) 
    {
        ...
        queue.add( new LocationDistance( source, 0.0 ) );
        ...
        while (!queue.isEmpty()) 
        {
            ...
            for (Edge edge : graph.getNeighbors(currentLocation)) 
            {
                ...
                if(newDistance < distances.get(neighbour)) 
                {
                    ...
                    queue.add( new LocationDistance( neighbour, newDistance ) );
                }
            }
        }
    ...
    public List<Location> getShortestPath( Location source, Location destination ) 
    {
        ...
        while(!queue.isEmpty()) 
        {
            ...
            for(Edge edge : graph.getNeighbors(currentLocation)) 
            {
                ...
                {
                    distances.put( neighbour, newDistance );
                    previous.put( neighbour, currentLocation );
    ...
    private List<Location> buildPath( Map<Location, Location> previous, Location destination ) 
    {
    ...
    {
        Map<Location, Double> distances = shortestDistances(source);

        return distances.get(destination);
    }

    // Return complete route information
    public RouteResult getRouteResult( Location source, Location destination ) 
    {
        List<Location> path = getShortestPath( source, destination );
        double distance = getShortestDistance( source, destination );
        int travelTime = 0;
        ...
        return new RouteResult( path, distance, travelTime );
    }
