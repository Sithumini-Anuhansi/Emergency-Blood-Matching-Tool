...
// Vertices : Location, Edges : Road connections between locations, Weight : Distance (km) and Travel Time (minutes)
...
public class Graph 
{
     ...
     */
    public void addRoad(Location source, Location destination, double distance, int travelTime) 
    {
        ...
        {
            throw new IllegalArgumentException( "Both locations must be added before creating a road." );
        }

        adjacencyList.get(source) .add(new Edge(destination, distance, travelTime));

        adjacencyList.get(destination) .add(new Edge(source, distance, travelTime));
    }
     ...
     * @param destination Destination location
     */
    public void removeRoad(Location source, Location destination) 
    {
        if (!adjacencyList.containsKey(source) || !adjacencyList.containsKey(destination)) 
        { return; }
    ...
    public void displayGraph() 
    {
        ...
        for (Location location : adjacencyList.keySet()) 
        {
            ...
            for (Edge edge : edges) 
            {
                System.out.printf( "   -> %-15s Distance: %.1f km | Time: %d mins%n",
                    edge.getDestination().getLocationName(), edge.getDistance(), edge.getTravelTime() );
            }
    ...
    public void displayNeighbors(Location location) 
    {
        ...
        System.out.println("\nNeighbours of " + location.getLocationName());

        for (Edge edge : adjacencyList.get(location)) 
        {
            System.out.printf( "-> %s (%.1f km)%n", edge.getDestination().getLocationName(), edge.getDistance() );
        }
    }

    // Return adjacency list needed later for Dijkstra.
