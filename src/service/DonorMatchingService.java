...
public class DonorMatchingService 
{
    ...
    public List<DonorMatch> findBestDonors( String bloodGroup, Location hospitalLocation ) 
    {
        ...
        {
            double distance = dijkstra.getShortestDistance( hospitalLocation, donor.getLocation() );

            // Ignore unreachable donors
            if (distance != Double.MAX_VALUE && !Double.isInfinite(distance)) 
            {
                matches.add( new DonorMatch( donor, distance ) );
            }
        }
